...
from dancingmodel.server.router.model_infer.infer_batch import InferBatch

from dancingmodel.common.configs.config import setting
from dancingmodel.models.bloom.model import BloomTpPartModel
from dancingmodel.models.llama.model import LlamaTpPartModel
from dancingmodel.models.llama_quantized.model import LlamaTpPartModelQuantized
from dancingmodel.models.llama_ppl.model import LlamaPPlTpPartModel
from dancingmodel.models.llama2_ppl.model import Llama2PPlTpPartModel
from dancingmodel.models.llama2.model import Llama2TpPartModel
from dancingmodel.models.starcoder.model import StarcoderTpPartModel
from dancingmodel.models.starcoder_ppl.model import StarcoderPPlTpPartModel
from dancingmodel.models.qwen.model import QWenTpPartModel
from dancingmodel.models.baichuan7b.model import Baichuan7bTpPartModel
from dancingmodel.models.baichuan13b.model import Baichuan13bTpPartModel
from dancingmodel.models.chatglm2.model import ChatGlm2TpPartModel
from dancingmodel.models.internlm.model import InternlmTpPartModel
from dancingmodel.models.peft.lora_adapter import LoraTpPartAdapter
from dancingmodel.models.peft.lora_unordered_batch_infer import LoraUnorderedBatchInfer
from dancingmodel.models.peft.lora_single_batch_infer import LoraPEFTBatchInfer
from dancingmodel.models.bmm.lora_bmm_infer import LoraBmmInfer
from dancingmodel.server.router.model_infer.infer_adapter import InferAdapter
from dancingmodel.server.router.model_infer.naive_infer_adapter import NaiveInferAdapter
from dancingmodel.utils.infer_utils import set_random_seed
...
class ModelRpcServer(rpyc.Service):
    ...
    def exposed_init_model(self, rank_id, world_size, weight_dir, adapter_dirs,
            ...
            if self.model_type == "bloom":
                ...
                if "num_key_value_heads" in model_cfg.keys():
                    if "ppl" not in mode:
                        ...
                        self.model = Llama2PPlTpPartModel(rank_id, world_size, weight_dir,
                                                          max_total_token_num,
                                                          load_way=load_way, mode=mode)
                else:
                    if "ppl" not in mode:
        ...
        if input_params.no_mem_pool:
            ...
            self.infer_adapter = InferAdapter.init(self.model.mem_manager, prefetch_stream,
                                                   world_size)
        # self.infer_adapter.load_adapters(self.adapters)
...
async def start_model_process(port, world_size):
    ...
    while repeat_count < 20:
        try:
            con = rpyc.connect("localhost", port, config={"allow_public_attrs" : True, "allow_pickle": True})
            break
