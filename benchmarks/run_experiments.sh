        ...
        for arrival in "${arrivals[@]}"; do
            for backend in "${backends[@]}"; do
                ...
                if [ "$backend" = "system" ]; then
                    ...
                    python run_exp_new.py --trace-file-path experiment_traces/${distribution}_${arrival}_${qps}.0_${duration}.csv --servers ${servers} --output ${distribution}_${arrival}_${qps}.0_${duration}_${backend}.txt --backend ${backend} --warmup-time 360 --warmup-requests ${warmup_requests}
                    rc=$?
                    if [ $rc -ne 0 ]; then
                    ...
                    python run_exp_new.py --trace-file-path experiment_traces/${distribution}_${arrival}_${qps}.0_${duration}.csv --servers ${servers} --output ${distribution}_${arrival}_${qps}.0_${duration}_${backend}.txt --backend ${backend} --warmup-time 60 --warmup-requests ${warmup_requests}
                    rc=$?
                    if [ $rc -ne 0 ]; then
                        echo "[$(date -u +'%Y-%m-%dT%H:%M:%SZ')] FAILED qps=${qps} distribution=${distribution} arrival=${arrival} backend=${backend} exit_code=${rc}" >> ../../outputs/run_log.txt
                    fi
                ...
                cp ${distribution}_${arrival}_${qps}.0_${duration}_${backend}.txt ../../outputs
                cp fine_${distribution}_${arrival}_${qps}.0_${duration}_${backend}.txt ../../outputs
