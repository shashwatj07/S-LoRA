...
def load_probability_sums(server_maps_dir: str):
    ...
    if not probability_sums_by_step:
        raise ValueError(f"No probability_sum_step_*.json files found in {server_maps_dir}")
...
async def benchmark_from_servermaps(
    ...
    for req in input_requests:
        ...
        if adapter_servers is None:
            raise ValueError(f"No servers found for adapter {req.adapter_dir} at step {step_idx}")
...
def run_exp(
    ...
    if server_maps_dir:
        if backend == "system":
            server_maps_by_step, _, max_step_idx = load_server_maps(server_maps_dir)
            probability_sums_by_step = load_probability_sums(server_maps_dir)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--server-maps-dir", type=str, default=None,
